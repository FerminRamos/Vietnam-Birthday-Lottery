...
Re-organize the data into monthly categories with counts of hits/misses per month.
...
## 1a. What month was best to have been born?
### In General, months with more days = Higher chances that someone else gets picked.
...
* Since `Feb` has the least number of days, with ``r ms_total_days %>% filter(month == "Feb") %>% pull(total_days)`` days. Individuals born in `Feb`, are ``r draft_prob("Feb")``% likely to be drafted. 

* On the contrary, since `Dec` has one of the most number of days, with ``r ms_total_days %>% filter(month == "Dec") %>% pull(total_days)`` days. Individuals born in `Dec` are ``r draft_prob("Dec")``% likely to be drafted. 

Here's a breakdown of the probability for each month:
